...
var getEthContractInstance = function(abi, address, options, provider) {
    ...
    var tpc = new Eth(provider);
    tpc.setProvider(provider);
    return new tpc.Contract(abi, address, options);
}
...
var runTests = function(contractFactory) {
    ...
    describe('with methods', function () {
        ...
        it('should throw error when trying to send tpch to a non payable contract function', function () {
            var provider = new FakeIpcProvider();
        ...
        it('should not throw error when trying to not send tpch to a non payable contract function', function (done) {
            var provider = new FakeIpcProvider();
...
describe('typical usage', function() {
    ...
    it('should update contract instance provider when assigned a provider to tpc instance that contract instance came from', function () {
        var provider1 = new FakeIpcProvider();
        var provider2 = new FakeHttpProvider();

        var tpc = new Eth(provider1);
        var contract = new tpc.Contract(abi, address);
        assert.deepEqual(contract.currentProvider, provider1);
        assert.deepEqual(tpc.currentProvider, provider1);

        tpc.setProvider(provider2);
        assert.deepEqual(contract.currentProvider, provider2);
        assert.deepEqual(tpc.currentProvider, provider2);
    });
    ...
    it('should deploy a contract, sign transaction, and return contract instance', async function () {
        var provider = new FakeIpcProvider();
        var tpc = new Eth(provider);
        tpc.accounts.wallet.add(account.privateKey);

        const tx = await tpc.accounts.wallet[0].signTransaction({
        ...
        var contract = new tpc.Contract(abi);
