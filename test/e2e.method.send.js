...
describe('method.send [ @E2E ]', function () {
    ...
    describe('http', function () {
        ...
        describe('transactionPollingTimeout', function(){
            ...
            it('is configurable for web3.tpc methods', async function(){
                web3.tpc.transactionPollingTimeout = .25;

                try {
                    await web3.tpc.sendTransaction({
                        from: accounts[0],
                        to: accounts[1],
                        value: web3.utils.toWei('1', 'tpch'),
                        gas: 21000,
                        gasPrice: 1
                    });
                    assert.fail();
    ...
    describe('ws', function () {
        ...
        it('fires the confirmation handler', function () {
            return new Promise(async (resolve, reject) => {
                ...
                var startBlock = await web3.tpc.getBlockNumber();
                ...
                await instance
                    ...
                    .on('confirmation', async (number, receipt) => {
                        if (number === 1) { // Confirmation numbers are zero indexed
                            var endBlock = await web3.tpc.getBlockNumber();
