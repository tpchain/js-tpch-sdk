...
describe('contract.events [ @E2E ]', function() {
    ...
    it('can immediately listen for events in the past', async function(){
        ...
        const latestBlock = await web3.tpc.getBlockNumber();
        ...
        web3 = new Web3('ws://localhost:' + port);
        const newInstance = new web3.tpc.Contract(Basic.abi, instance.options.address);
    ...
    it('hears events when subscribed to "logs" (emitter)', function(){
        return new Promise(async function(resolve, reject){
            ...
            const subscription = web3.tpc.subscribe(
                "logs",
                {
    ...
    it('handles child events with shadowed signatures', async function(){
        ...
        contract = new web3.tpc.Contract(Child.abi, options);
        const child = await contract.deploy().send({from: accounts[0]});

        options.data = Parent.bytecode;
        contract = new web3.tpc.Contract(Parent.abi, options);
    ...
    it('backfills missed events when auto-reconnecting', function(){
        ...
        const shadow = new _web3.tpc.Contract(Basic.abi, instance.options.address);
        ...
        return new Promise(async function (resolve) {
            ...
            const finalBlock = await _web3.tpc.getBlockNumber();
