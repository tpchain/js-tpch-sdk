...
The ``web3.tpc.subscribe`` function lets you subscribe to specific events in the blockchain.
...
.. code-block:: javascript
    ...
    web3.tpc.subscribe(type [, options] [, callback]);
...
.. code-block:: javascript
    ...
    var subscription = web3.tpc.subscribe('logs', {
        address: '0x123456..',
        topics: ['0x12345...']
...
.. code-block:: javascript
    ...
    web3.tpc.subscribe('logs', {} ,function(){ ... });
...
.. code-block:: javascript
    ...
    web3.tpc.subscribe('pendingTransactions' [, callback]);
...
.. code-block:: javascript
    ...
    var subscription = web3.tpc.subscribe('pendingTransactions', function(error, result){
        if (!error)
            console.log(result);
    })
...
.. code-block:: javascript
    ...
    web3.tpc.subscribe('newBlockHeaders' [, callback]);
...
.. code-block:: javascript
    ...
    var subscription = web3.tpc.subscribe('newBlockHeaders', function(error, result){
        if (!error) {
            console.log(result);
...
.. code-block:: javascript
    ...
    web3.tpc.subscribe('syncing' [, callback]);
...
For the structure of a returned event ``Object`` see :ref:`web3.tpc.isSyncing return values <eth-issyncing-return>`.
...
.. code-block:: javascript
    ...
    var subscription = web3.tpc.subscribe('syncing', function(error, sync){
        if (!error)
            console.log(sync);
    })
...
.. code-block:: javascript
    ...
    web3.tpc.subscribe('logs', options [, callback]);
...
For the structure of a returned event ``Object`` see :ref:`web3.tpc.getPastEvents return values <eth-getpastlogs-return>`.
...
1. ``Object|Null`` - First parameter is an error object if the subscription failed.
2. ``Object`` - The log object like in :ref:`web3.tpc.getPastEvents return values <eth-getpastlogs-return>`.
