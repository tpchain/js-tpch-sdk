...
.. note:: When called on the umbrella package ``web3`` it will also set the provider for all sub modules ``web3.tpc``, ``web3.shh``, etc. EXCEPT ``web3.bzz`` which needs a separate provider at all times.
