...
async function setupENS(web3) {
    ...
    const accounts = await web3.tpc.getAccounts();
    const from = { from: accounts[0] };

    // ENS
    options.data = ENSArtifact.bytecode;
    const ENS = new web3.tpc.Contract(ENSArtifact.abi, options)
    ...
    const PublicResolver = new web3.tpc.Contract(PublicResolverArtifact.abi, options)
    ...
    const FIFSRegistrar = new web3.tpc.Contract(FIFSRegistrarArtifact.abi, options)
    ...
    const ReverseRegistrar = new web3.tpc.Contract(ReverseRegistrarArtifact.abi, options)
